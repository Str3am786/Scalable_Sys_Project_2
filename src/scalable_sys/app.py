...
from src.scalable_sys.rag.graph_rag import ManualGraphRAG
...
def _make_base_llm(cfg):
    ...
    elif cfg.backend in ("server", "rag"):
...
def get_llm(use_cache: bool = False):
    ...
    Returns:
      ...
      - ManualGraphRAG             when backend == "rag"
    """
    cfg = load_config()
    base_llm = _make_base_llm(cfg)

    if cfg.backend == "rag":
        llm: ManualGraphRAG | CachedLLM = ManualGraphRAG(
            llm=base_llm,
...
def main():
    ...
    #   - "inproc" (plain local gguf baseline)
