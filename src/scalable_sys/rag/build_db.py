...
@app.cell
def _(Path, pl):
    # <root>/src/scalable_sys/rag/build_db.py -> parents[3] == <root>
    _project_root = Path(__file__).resolve().parents[3]
    _data_dir = _project_root / "data"

    filepath = (_data_dir / "nobel.json").resolve()
...
@app.cell
def _(Path):
    # Save the Kuzu database into <project_root>/data/nobel.kuzu
    _project_root = Path(__file__).resolve().parents[3]
    _data_dir = _project_root / "data"

    db_path = (_data_dir / "nobel.kuzu").resolve()
...
@app.cell
def _(db_path, kuzu):
    # Connect to the Kuzu database in ./data
...
def _(conn, df):
    ...
    num_laureate_affiliation_rels = res10.get_as_pl()[
        "num_laureate_affiliation_rels"
    ][0]
...
def _(conn, df):
    ...
    num_country_affiliation_rels = res13.get_as_pl()[
        "num_country_affiliation_rels"
    ][0]
    print(
